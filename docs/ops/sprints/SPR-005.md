...
* **Provider clients scaffolding**
  - Added placeholder `GoogleClassifierClient`/`GoogleAssessClient` and `OpenRouterClassifierClient`/`OpenRouterAssessClient` (raise `NotImplementedError`, enforce API key presence) to unblock factory wiring.
