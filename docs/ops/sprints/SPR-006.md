# Sprint SPR-005 — Monitoring & Guardrails
...
- [ ] Conversation Archive (Simple Chat Storage) — <owner> (<estimate>)
   - [ ] Convex schema: add `conversations` table (session-scoped, simple turns)
   - [ ] Convex functions: `appendConversation`, `listBySession`
   - [ ] API: `POST /api/v1/conversations` (append), `GET /api/v1/conversations/:sessionId` (list)
   - [ ] Frontend: text chat writes user+assistant turns to archive
   - [ ] Backfill script (optional): migrate from `interactions` where feasible
   - [ ] Redaction: apply privacy filters before persist/log
   - [ ] Metrics & logs: emit ingest success/error, latency, payload size
...
- [ ] FEAT-023 — Privacy Redaction Pipeline (infra)
 - [ ] FEAT-031 — Conversation Archive (Simple Chat Storage) (data)
...
- [ ] Redaction verified in logs and events; no PII leakage
 - [ ] Conversation archive persists user/assistant turns by session; retrievable via API; basic retention policy documented
...
#### Conversation Archive Model (new)
- Table: `conversations`
  - `sessionId: string` — required
  - `turnIndex: number` — monotonically increasing per session
  - `messageId: string` — required
  - `role: "user" | "assistant"` — required
  - `text: string` — required
  - `ts: number` — event timestamp (ms since epoch)
  - `createdAt: number`
  - Optional: `modelId?: string`, `meta?: any`
- Indexes:
  - `by_session` (`sessionId`)
  - `by_session_turn` (`sessionId`, `turnIndex`)
- Functions:
  - `appendConversation(sessionId, messageId, role, text, ts)` → inserts with next `turnIndex`
  - `listBySession(sessionId, limit?)` → chronological transcript
- API:
  - `POST /api/v1/conversations` → append
  - `GET /api/v1/conversations/:sessionId` → list
- Notes:
  - Keep model simple; do not require `groupId`/`contentHash`
  - Redact/normalize text before storage per privacy policy
...
- [ ] Cost calc unit tests vs known inputs
 - [ ] Conversations: unit tests for Convex functions (append/list)
 - [ ] Conversations: API tests (200 happy path, validation errors)
 - [ ] Conversations: integration test from UI text chat (user+assistant turns stored and listed)
...
- `conversations_ingest_ok`, `conversations_ingest_err`
- Ingest latency, payload size histogram
- Logs: requestId, sessionId, messageId, role
...
- [ ] Rate limiting and idempotency considered for new/changed endpoints
 - [ ] Data retention policy documented for `conversations`; sampling/anonymization plan for training exports
 - [ ] Export tooling has access controls and audit logs
...
- [ ] Cross-links updated (PRD/Technical Overview)
 - [ ] Data model docs: `conversations` schema, indexes, constraints
 - [ ] Runbook: how to export conversations for training (privacy, sampling, approvals)
...
- 25/08/2025 Added Conversation Archive (Simple Chat Storage) tasks, tests, monitoring, hygiene, and documentation
