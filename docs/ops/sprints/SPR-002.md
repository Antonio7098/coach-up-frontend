...
### Algorithmic Details
- Approach: message-ingestion-driven decisions with an in-memory interaction state machine per `sessionId`.
  ...
  - Queue: enqueue `{ sessionId, groupId }` to background worker (in-memory `asyncio.Queue`) for rubric scoring and summary stub.
- Rubric Version: v1
