...
- __Key endpoints__
  ...
  - `GET  /metrics` — exposes lightweight service metrics
...
- [x] Unit tests
  ...
  - AI API (pytest)
    ...
    - `coach-up-ai-api/tests/test_assessments.py::test_metrics_endpoint_smoke` — `/metrics` endpoint shape
...
- [x] Contract tests
  - Next.js ↔ FastAPI AI — passing
    ...
    - Runner: `ui/playwright.config.ts` with dual webServer setup (uvicorn + Next)
      ...
      - Env: `CSS_TRANSFORMER_WASM=1`, `AI_API_BASE_URL=http://127.0.0.1:8001`, `MOCK_CONVEX=1`, `PERSIST_ASSESSMENTS_SECRET` injected consistently for server and tests
...
- Date: 21/08/2025 — Issues: (1) Ingestion contract returned `latestGroupId=null`; (2) Convex finalize returned 401 Unauthorized — Impact: Playwright E2E failures
  - Detection: Playwright contract tests for `/api/messages/ingest` polling AI API `/assessments/{sessionId}`; Convex finalize smoke
  - Fixes:
    - AI API (`coach-up-ai-api/app/main.py`):
      - Immediately persist placeholder assessment on `start`/`continue` to expose `latestGroupId` promptly
      - `GET /assessments/{sessionId}` decodes percent-encoded IDs and checks both raw/decoded keys; runs sync compute fallback when possible
      - Added structured debug logging for lookups (`assessments_get_lookup`, `assessments_get_state`)
    - UI finalize route (`ui/src/app/api/assessments/convex/finalize/route.ts`):
      - Skip Authorization enforcement when `MOCK_CONVEX=1` to avoid 401s in tests
    - Playwright config (`ui/playwright.config.ts`):
      - Run Next.js on port 3100 with a fresh server (`reuseExistingServer=false`)
      - Inject `PERSIST_ASSESSMENTS_SECRET` consistently into Next.js server env and Playwright tests
      - Ensure env values are typed as strings to satisfy Playwright config typing
  - Outcome: All E2E tests pass locally (16/16); ingestion polling sees non-null `latestGroupId` and summary; finalize no longer 401s in mock mode
  - Follow-up: Consider persisting immediate placeholders to Convex; keep debug logs until CI is stable

- Date: 21/08/2025 — Issue: Convex routes returned 502 (Bad Gateway) during real-convex E2E
