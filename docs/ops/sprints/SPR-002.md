...
- [ ] Multi-turn Assessment Job — (<estimate>)
   - [ ] LLM Boundary Classifier integration (LLM-first; thresholds; caching by `messageId`)
   - [ ] Message ingestion endpoint (`POST /messages/ingest`) + per-session in-memory state machine (idempotent by `messageId`)
   - [ ] Grouping lifecycle: create/attach/finalize `groupId`; handle one-off assessments
   - [ ] Enqueue to Assessment Agent with idempotency key `(sessionId, groupId)`; retries/backoff; concurrency limits
   - [ ] Observability: decisions/confidences, queue depth, job latencies, cost metrics
...
- [ ] FEAT-031 — Durable Job Queue (infra)
