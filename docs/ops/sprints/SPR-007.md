...
- [ ] Finalize route accepts v2-only payload: requires level, persists per-skill, updates levels
...
- [ ] V2-only API finalize route — <owner> (1.5d)
  - [ ] Replace v1 schema with v2-only validation in `ui/src/app/api/assessments/convex/finalize/route.ts`
  - [ ] Validate shared secret header and `rubricVersion === "v2"`
  - [ ] Require `summary.skillAssessments[].level` (0–10), `skillHash`, and arrays `metCriteria/unmetCriteria/feedback`
  - [ ] Resolve `userId` via `convex/functions/sessions:getBySessionId` (by `sessionId`)
  - [ ] For each skill item: call Convex to write per-skill row and trigger level update
...
- [ ] Mock Convex + Test Harness — <owner> (1d)
  - [ ] Extend `ui/src/app/api/lib/mockConvex.ts` with new mutations/queries
  - [ ] Add fixtures for skills and sessions to cover user resolution and level updates
