...
- 23/08/2025 Monitoring (Grafana/Prometheus): added UI dashboards (`ui-api-storage-presign.json`, `ui-api-transcripts.json`), provisioning, and `infra/monitoring/docker-compose.yml` for local stack
 ...
 - 24/08/2025 UI: added Chat Voice Mode page `ui/src/app/chat/voice/page.tsx` with mic capture, presign+upload, STT call, and TTS playback; linked from `ui/src/app/chat/page.tsx`; `.env.example` updated with `NEXT_PUBLIC_ENABLE_VOICE` and 2165r`NEXT_PUBLIC_VOICE_MAX_UTTERANCE_MS`
